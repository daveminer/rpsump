...
async fn get_schedule_success() {
    ...
    let schedules = schedules_response.json::<Vec<IrrigationSchedule>>().await.unwrap();
    let schedule_response = app.get_irrigation_schedule(token.to_string(), schedules[0].id).await;
    let status = schedule_response.status();
...
async fn delete_schedule_success() {
    ...
    let schedules = schedule_response.json::<Vec<IrrigationSchedule>>().await.unwrap();
    let response = app.delete_irrigation_schedule(token.to_string(), schedules[0].id).await;
...
async fn delete_schedule_not_found() {
    ...
    // Assert
    assert!(schedule_response["message"] == "Error deleting irrigation schedules: Record not found");
    assert!(status == 400);
}
...
async fn patch_schedule_success() {
    ...
    let response = app.patch_irrigation_schedule(token.to_string(), update.id, body).await;
    let status = response.status();
...
async fn patch_schedule_not_found() {
    ...
    let response = app.patch_irrigation_schedule(token.to_string(), 1, body).await;
    let status = response.status();
    let updated_schedule: String = response.text().await.unwrap();
    println!("UP: {:?}", updated_schedule);
...
async fn post_schedule_success() {
    ...
    let status = schedule_response.status();
    let new_schedule = schedule_response.json::<IrrigationSchedule>().await.unwrap();
